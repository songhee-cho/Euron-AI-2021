...
Original file is located at
    https://colab.research.google.com/drive/1RJfQWi0RjumyN_aP-8ZnzxzkLLa3hkhH
"""
...
FOLDERNAME = 'stanford-cs231n/assignments/assignment3'
...
When we update the image according to the gradient, of course we need to update the gradient of the same size as the image, that is, the gradient map of 3 * H * W size, but **our saliency map is in the C dimension. The maximum value is taken, so the size is H*W, so it is not allowed.**

First, the *saliency map* has a different dimension but this could be fixed by broadcasting over the RGB channels but that already suggests it can't be used in place of the true gradient.

Secondly, saliency is converted to grayscale by taking the maximum over the RGB. This means that gradient update steps apply the same update to each of the channels and every pixel would be effectively restrained from changing the RGB channels independently.

Third, *saliency calculation* performs absolute value before taking the maximum over the channels. So the updates would never be able to **decrease** anything and could **only increase the pixel intensity values and never decrease it.**
