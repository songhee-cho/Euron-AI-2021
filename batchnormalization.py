...
Original file is located at
    https://colab.research.google.com/drive/1eHuOqOnDG0LmKTKByq21Y-zAA6Rds-2u
"""
...
FOLDERNAME = 'stanford-cs231n/assignments/assignment2'
...
## Answer:
Models with batch normalization are more robust to the scale of the initialization noise and converge to higher validation accuracy then non-normalized ones when trained for a fixed number of epochs. If the network would be trained to achieve a fixed accuracy the batch normalized one would reach the target after a fewer epochs.Deep networks are by their nature ill-conditioned. A small change in the input leads to a large change in the output layers due to repeatedly applied non-linearities. Batch normalization removes this to some extent and for larger noise scales the trainig still remains stable and the network is able to learn because the gradients of weights used for updating are smaller. Non-normalized networks diverge for initial noise scale about ~$10^{-1}$ while the normalized ones can be trained for weights up to ~$10^0$.The robustnest comes from the ability of batch normalized networks to de-learn the ill-posed initialization scale thanks to it's built-in scale parameter γ. The offset β affects the result only for layers with smaller number of neurons. Random weights are more prone to accumulate a non-zero mean value in this case.
...
## Answer:
Positive effects of batch normalization depend on the batch size. Training with small batch size has worse performance than non-normalized ones. Until the batch size of about 50 the batch normalized network performs worse than the non-normalized one with a smaller batch size and using the normalization has a detrimental effect.Ideally one would do batch normalization over the entire dataset to get the expected mean and variance of each neron activation. This is however impractical so only a sub-set of the training set is used to estimate the mean and the variance. With smaller batch sizes the estimation gets noisier and causes the running values stored at each layer to oscillate. An  ill-sampled batch with a large activation offset can cause a difficulty for the following batch since that will generaly try to update the bias and variance weights in the opposite direction.
...
Tests showing the performance of layer normalization with different hidden sizes and regularization values can be seen in the next cells.
